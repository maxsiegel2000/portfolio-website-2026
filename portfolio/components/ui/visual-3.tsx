...
const Layer3: React.FC<{ color: string }> = ({ color }) => {
  return (
    <div className="ease-[cubic-bezier(0.6, 0.6, 0, 1)] absolute inset-0 z-[6] origin-bottom translate-y-full opacity-0 transition-all duration-500 group-hover/animated-card:translate-y-0 group-hover/animated-card:opacity-100 group-hover/animated-card:scale-120">
      <svg
        viewBox="0 0 356 180"
        preserveAspectRatio="none"
        className="h-full w-full"
...
const Layer2: React.FC<{ color: string }> = ({ color }) => {
  return (
    ...
    >
      <div className="ease-[cubic-bezier(0.6, 0.6, 0, 1)] absolute inset-0 z-[7] flex w-full translate-y-full translate-x-[10%] items-start bg-transparent p-4 transition-transform duration-500 group-hover/animated-card:translate-y-0">
